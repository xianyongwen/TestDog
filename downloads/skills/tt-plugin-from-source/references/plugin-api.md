...
- **动画/过渡竞态**：滚轮、拖拽、展开类组件常在动画期间忽略点击（如 vant picker 源码
  `onClickOption` 有 `moving` 守卫）——点完等「状态类落定」（如 `--selected` 类）再进行下一步，
  未落定时**重试一次再报错**，绝不静默带错值前进（错值会被 verify 拦下，但静默重试会拖长动作甚至超时）
- **只读触发器一律 `preferFill: false`**：弹层选择器的触发输入框多为 readonly，fill 必然失败——
  声明 true 只会白耗一次尝试；「fill 优先」只适用于可输入且 fill+Enter 能提交的控件（先实测）
- **React 系组件的页内事件手法**（合成事件走页内闭环时）：
  - 框架在 `onMouseDown` 开弹层的（如 MUI Select），合成 `el.click()` 打不开——须派发完整
    mousedown/mouseup/click 序列；detect/candidates 里 id 与 role 的落点也要实测（MUI 的
    role=combobox 与 id 同落在 `.MuiSelect-select` 展示层自身，`querySelector` 找后代类名会扑空）
  - React 受控输入赋值必须用**原生 setter 技巧**：直接改 `input.value` 会同步 React 的 value
    tracker，此后派发 input 事件不触发 onChange——用
    `Object.getOwnPropertyDescriptor(HTMLInputElement.prototype, 'value').set.call(input, v)`
    写值后再 `dispatchEvent(new Event('input', { bubbles: true }))`（testing-library 同款）
  - 拖拽组件对合成 mousemove 有**按键完整性守卫**：MUI Slider 的 handleTouchMove 把
    `buttons === 0` 的 mousemove 视为「mouseup 被吞」并取消拖拽——合成 mousemove 必须带
    `buttons: 1`（mousedown 带 `button: 0, buttons: 1`），否则拖拽静默失效
