...
const STUCK_MUTATING = new Set(['scroll', 'upload', 'goto', 'click', 'fill', 'press', 'check', 'select', 'component_action', 'act', 'batch_actions']);
...
export function stuckSig(name: string, args: Record<string, unknown>): string | null {
  ...
  if (name === 'scroll') return `scroll ${String(args.target)} ${sel} ${String(args.mode)} ${String(args.axis ?? 'y')} ${String(args.distance ?? '')}`;
