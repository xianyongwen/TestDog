...
interface Run {
  ...
  scriptId?: string
  meta?: { usage?: TokenUsage; stepUsages?: Record<number, TokenUsage> } | null
}
interface CaseDetail {
  ...
  project: {
    ...
    baseUrl?: string
    envVars?: { key: string }[]
    loginConfigs?: { id: string; name: string; isDefault: boolean }[]
  }
  scripts: Script[]
  runs: Run[]
}
...
export default function TestCaseDetail() {
  ...
  useEffect(() => {
    return ws.on(msg => {
      ...
      if (msg.type === 'run:step') {
        ...
        if (su)
          parts.push(
            t('caseDetail.usageText', {
              total: fmtToken(su.totalTokens),
              cached: fmtToken(su.cachedTokens),
            })
          )
        setLogItems(prev => [
          ...prev,
          {
            ...
            title: (
              <span>
                {t('caseDetail.stepLabel', { index: msg.index })}{' '}
                <ActionTag action={(msg as any).action ?? ''} />{' '}
                <Tag color={(msg as any).healed ? 'purple' : 'default'}>
                  {(msg as any).healed ? t('caseDetail.healed') : msg.status}
                </Tag>
              </span>
            ),
            ...
            desc: u ? (
              <>
                {t('caseDetail.usageText', {
                  ...
                  cached: fmtToken(u.cachedTokens),
                })}
  ...
  const saveAsVersion = async () => {
    if (!caseId) return
    await http.post(`/api/test-cases/${caseId}/scripts`, {
      steps: editSteps,
  ...
  const exportScript = async () => {
    if (!selScript || !data) return
    const hide = message.loading('正在打包用例和上传文件…', 0)
    try {
      const payload = await http.post(
        `/api/projects/${encodeURIComponent(data.project.id)}/test-cases/export`,
        {
          format: 'testcase',
          version: 1,
          title: data.title,
      ...
      anchor.download = `${safeName}-v${selScript.version}.testcase`
  ...
  const runSteps = async (steps: TestStep[], scriptId?: string) => {
    ...
    setSelRunId(null)
    setRunDetail(null)
    const { jobId } = await http.post<{ jobId: string }>('/api/runs', {
      testCaseId: caseId,
      scriptId,
      steps,
      selfHeal: true,
    ...
    <div className="flex h-[calc(100vh-40px)] flex-col">
      <div className="mb-4 flex shrink-0 items-center gap-2">
        <Button
          ...
          onClick={() => nav(`/projects/${data.project.id}`)}
        ></Button>
      ...
      <Card className="mb-4 shrink-0">
        <Descriptions size="small" column={2}>
          ...
          <Descriptions.Item label={t('caseDetail.baseUrl')}>
            {data.project.baseUrl || '-'}
          </Descriptions.Item>
          <Descriptions.Item label={t('caseDetail.description')} span={2}>
            {data.description || '-'}
          </Descriptions.Item>
        </Descriptions>
        <Space className="mt-3" wrap>
          <Button type="primary" onClick={() => nav(`/cases/${caseId}/generate`)}>
            <ThunderboltOutlined className="mr-1.5" />
            {t('caseDetail.aiGenerate')}
          </Button>
          <Button type="primary" onClick={() => nav(`/cases/${caseId}/record`)}>
            <VideoCameraOutlined className="mr-1.5" />
            {t('caseDetail.manualRecord')}
          </Button>
          <Button
            type="primary"
            disabled={!selScript}
            onClick={() => selScript && runSteps(selScript.steps, selScript.id)}
          >
            <PlayCircleOutlined className="mr-1.5" />
            {t('caseDetail.runCurrent')}
          </Button>
          ...
          <span>
            ...
            <Select
              ...
              options={data.project.loginConfigs?.map(c => ({
                value: c.id,
                label: c.name + (c.isDefault ? t('common.defaultTag') : ''),
              }))}
            />
          </span>
        </Space>
          ...
          title={
            <span className="inline-flex items-center gap-3">
              ...
              <Button
                ...
                danger
                icon={<StopOutlined />}
                onClick={() => jobIdRef.current && ws.cancel(jobIdRef.current)}
              >
                  ...
                  {selScript ? (
                    <>
                      <Space className="mb-2 shrink-0" wrap>
                        <div className="flex h-8 items-center justify-center rounded-md border border-accent bg-accent-subtle px-2.5 text-accent">
                          v{selScript.version}
                        </div>
                        <Button icon={<SaveOutlined />} onClick={save}>
                          {t('common.save')}
                        </Button>
                        <Button icon={<SaveOutlined />} onClick={saveAsVersion}>
                          {t('caseDetail.saveAsVersion')}
                        </Button>
                        <Button
                          type="primary"
                          icon={<PlayCircleOutlined />}
                          onClick={() => runSteps(editSteps, selScript.id)}
                        >
                          {t('common.run')}
                        </Button>
                        {selScript.intent && (
                          <Button onClick={() => setIntentOpen(true)}>
                            {t('generate.intent.title')}
                          </Button>
                        )}
                        <Button icon={<DownloadOutlined />} onClick={exportScript}>
                          {t('common.export')}
                        </Button>
                      </Space>
                      <Modal
                        open={intentOpen && !!selScript.intent}
                        title={t('generate.intent.title')}
                        onCancel={() => setIntentOpen(false)}
                        footer={null}
                        width={800}
                      >
                        <div className="max-h-[65vh] overflow-y-auto">
                          {selScript.intent && <TestIntentEditor value={selScript.intent} />}
                        </div>
                      </Modal>
                      <StepsTable
                        fill
                        ...
                        envVarKeys={data.project.envVars?.map(v => v.key)}
                        pickStartUrl={data.project.baseUrl}
                        pickLoginConfigId={loginConfigId}
                        pickLoginConfigs={data.project.loginConfigs}
                        pickProjectId={data.project.id}
                ...
                <div className="flex w-[480px] shrink-0 flex-col min-h-0">
                  <Space className="mb-2 shrink-0" wrap>
                    <span className="text-ink-2">
                      {t('common.total', { count: data.runs.length })}
                    </span>
                    <Popconfirm
  ...
  const adopt = async (stepIndex: number) => {
    ...
    try {
      const res = await http.post<{
        ...
        version: number
        stepIndex: number
      }>(`/api/runs/${runId}/adopt`, { stepIndex })
      ...
      <SortableTable
        ...
        columns={[
          ...
          },
          {
            ...
            dataIndex: 'status',
            width: 100,
            render: v => <StepStatusTag status={v} />,
          },
          {
            title: t('caseDetail.healed'),
            dataIndex: 'healed',
            width: 70,
            render: v => (v ? <Tag color="green">{t('caseDetail.healedYes')}</Tag> : '-'),
          },
          {
              ...
              <span>
                ...
                {r.healed && r.healedLocator ? (
                  ...
                  <Tooltip title={t('caseDetail.healedLocator')}>
                    <code className="ml-1 rounded bg-subtle px-1 py-0.5 text-[11px] break-all text-accent">
                      {describeLocator(r.healedLocator)}
                    </code>
                  </Tooltip>
                ) : null}
              </span>
                  ...
                  <div className="flex flex-wrap gap-4">
                    {r.screenshot ? (
                      <div className="min-w-[200px]">
                        ...
                        <Image
                          src={`${apiBase}/api/screenshots/${r.screenshot}`}
                          width={280}
                          className="max-h-[400px] border border-line-subtle object-contain"
                        />
                      </div>
                    ) : null}
                    {r.consoleLog ? (
