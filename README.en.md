...
- **More**: single / bulk import & export of `.testcase` files; drag-sort cases, batch-set default script versions, and export a **test report** (Excel with per-run step details / console / network / token) after batch runs; three skill packages + test-friendly code rules for coding agents (generate-testcase for importable cases, tt-component-plugin for DOM-driven plugin development, tt-plugin-from-source for generating plugins from project source); system variables for unique test data (random phone / email / ID number); bilingual UI (Chinese / English).
